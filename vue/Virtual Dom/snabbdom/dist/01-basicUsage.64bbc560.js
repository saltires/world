...
var patch = (0, _snabbdom.init)([]); // 使用 h 函数创建虚拟 DOM，这里 h 函数的参数：
// 第一个参数：标签 + 选择器
// 第二个参数：如果是字符串的话就是标签中的内容

var vnode = (0, _snabbdom.h)('div#container.cls', 'hello world');
var app = document.querySelector('#app'); // 调用 patch 时，第一个参数可以是 DOM 元素，内部会把 DOM 元素转为 VNode，如果第一个参数直接就是 VNode，那就无需转换了
// 第二个参数：VNode
// patch 函数调用时，会去更新真实的 DOM，也就是把第二个参数的 VNode 渲染到真实界面上
// 返回值： VNode

var oldVNode = patch(app, vnode);
var vnode1 = (0, _snabbdom.h)('div', 'hello change worlf');
...
//# sourceMappingURL=/01-basicUsage.64bbc560.js.map